...
public class ChallanScreen extends JDialog
{
	...
	JButton btnSubmit, btnReset, btnBack, btnPrint, btnPayableAmount;
	...
	public ChallanScreen( StudentList studentList, CourseInfo course ) 
	{
		...
		if( studentList.concessionEligible == 'N' )
			v = "Eligible for Concession : NO ";
		else
		if( studentList.concessionEligible == 'Y' )
			v = "Eligible for Concession : YES ";
			
		lblConcessionEligible = new JLabel( v );
		...
		btnPayableAmount = new JButton( "Calculate Payable Amount" );
		btnPayableAmount.setBounds( 350, 450, 200, 30 );
		btnPayableAmount.addActionListener( new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				new PayableFeesReport( studentList.studRegNum, course.courseNum );
		...
		lblBranchName.setBounds(500, 550, 150, 30);
		
		txtBranchName = new JTextField( 250 );
		txtBranchName.setBounds( 600, 550, 250, 30);
		
		lblDrawerName = new JLabel( "Drawer Name :");
		lblDrawerName.setBounds( 10, 600, 100, 30);
		
		txtDrawerName = new JTextField( 250 );
		txtDrawerName.setBounds( 200, 600, 250, 30);
		...
		btnPrint.setBounds( 320, 650, 100, 30);
		...
		btnSubmit.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				 ...
				 System.out.println( txtCurrentInstallment.getText() + "======");
				 
				if( txtCurrentInstallment.getText().equals("") )
				{
					JOptionPane.showMessageDialog(ChallanScreen.this, "Kindly Enter Valid Installment Amount", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				float currentInstallment = Float.parseFloat( txtCurrentInstallment.getText().replaceAll(",", "") );
		...
		btnPrint.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) 
			{
				...
				// Student Copy 
				buffer.append( lblHeading.getText() + "\n" );
				buffer.append( lblCollegeName.getText() + "\n" );
				buffer.append( lblCopyName.getText() + "\n"  );
				buffer.append( "Challan Number : " + lblChallanNumberVal.getText() + "\n" );
				buffer.append( "Name : " + lblStudNameVal.getText() + "\n" );
				buffer.append( "Course Name : " + lblCourseNames.getText() + "\n"  );
				
				String yearFrom, yearTo = null;
				yearFrom = comboYearStart.getSelectedItem().toString();
				yearTo = comboYearEnd.getSelectedItem().toString();
				
				buffer.append( "Academic Year " + yearFrom + " to " + yearTo + "\n" );
				buffer.append( "Course Fees & Balance : " + lblCourseFeesVal.getText() + "\n" );
				buffer.append( "Current Installment : " + txtCurrentInstallment.getText() + "\n" );
				
				DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
				LocalDate date = LocalDate.now();
				String dateStr = date.format(formatter);
				buffer.append( "Installment Date : " + dateStr + "\n" );
				buffer.append( "Particulars : " );
				if( particulars.getSelectedIndex() == 0 )
					buffer.append( "CASH \n " );
				else
				{
					buffer.append( "CHEQUE/DD \n" );
					buffer.append( "Bank Name : " + txtBankName.getText() + "\n" );
					buffer.append( "Branch Name : " + txtBranchName.getText() + "\n" );
					buffer.append( "Drawer Name : " + txtDrawerName.getText() + "\n" );
				}
				...
				if( particulars.getSelectedIndex() == 0 )
				{
					...
					buffer.append( denominationValues + "\n " );
				}
				
				buffer.append( "Signatures : " + "\n" );
				
				buffer.append( "\n \n \n \n \n " );
				buffer.append( "Bank - Incharge : " + "\t" );
				buffer.append( "Office - Incharge : " + "\t" );
				buffer.append( "Depositor Sign  : " + "\t" );
				
				
				
				
				JTextArea jTextArea = new JTextArea( buffer.toString() );
