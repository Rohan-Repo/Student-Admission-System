...
public class DBOperations extends BaseConnection
{
	...
	public static boolean addStudentInfo( StudentInfo studentInfo )
	{
		...
		try
		{
			...
			if( row == row2 )
				msg = "Info. added for " + studentInfo.studName + " with Registration Number #" + studentInfo.studRegNum ;
			else
			{
				msg = "Discrepency";
				retVal = false;
	...
	public static DefaultTableModel populateTable( int courseNo ) 
	{
		...
		/*
		String query = "select studRegNum, studName, courseName from " +
		...
		String query2 = "select studRegNum AS RegistrationNumber, studName AS StudentName,"  + 
						" getCourseName( student_master.courseNum ) AS CourseName from student_master " +
						" where courseNum = ? ";
...
//			DecimalFormat decimalFormat = new DecimalFormat("##,###.00");
	...
	public static DefaultTableModel retrieveStudentReport( String sortBy ) 
	{
		...
		try
		{
			...
			if( sortBy.equals("student") )
			{
				...
				{
					query = "select getCourseName(courseNum) AS CourseName, studRegNum AS RegistrationNumber, " + 
							"getStudName(studRegNum) AS StudentName from stud_qualifications " + 
			...
			if( resultSet != null )
			{
				...
				while ( resultSet.next() ) 
				{
					...
					if( sortBy.equals("balance") || sortBy.equals("advance") )
					{
			...
			System.out.println("RowCnt =" + vectorRowData.size() + "ColCnt=" + vectorColumnNames.size());
	...
	public static DefaultTableModel retrievePayableFeesReport(int studRegNum, int courseNum) 
	{
		...
		String query =  " select DISTINCT studName AS StudentName, "
				+ "getCourseName( student_master.courseNum ) AS CourseName, totalBalance, totalAdvance, " +
				"getPayableFeesNew( student_master.studRegNum, student_master.courseNum ) AS PayableFees " +
				" from student_master, balance_fees, advance_fees WHERE " +
				" totalBalance = ( select sum(totalBalance) from balance_fees where studRegNum =  ? ) AND " +
				" totalAdvance = ( select sum(totalAdvance) from advance_fees where studRegNum = ? ) " +
				" AND student_master.studRegNum = ? AND student_master.courseNum = ? ";
		...
		try
		{
			...
			preparedStatement = baseConnection.dbConnection.prepareStatement( query );
			preparedStatement.setInt( 1, studRegNum );
			preparedStatement.setInt( 2, studRegNum );
			preparedStatement.setInt( 3, studRegNum );
			preparedStatement.setInt( 4, courseNum );
			resultSet = preparedStatement.executeQuery();
